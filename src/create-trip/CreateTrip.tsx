  ...
  return (
    <div className="sm:px md:px lg:px-56 xl:px-96 2xl:px-96 px-5 mt-10">
      ...
      <div className="flex flex-col gap-2">
        <label htmlFor="trip-destination" className="text-gray-500 text-[20px]">
          Trip Destination
        </label>
      ...
      <div className="flex flex-col gap-2">
        <label htmlFor="trip-dates" className="text-gray-500 text-[20px]">
          Trip Dates
        </label>
        <Input
          type="date"
          ...
          onChange={(e) => handleInputChange("startDate", e.target.value)} // Update start date
        />
        <Input
          type="date"
          id="trip-end-date"
          className="border border-gray-300 rounded-md p-2"
          onChange={(e) => handleInputChange("endDate", e.target.value)} // Update end date
        />
      ...
      <div>
        <div className="text-xl my-3 font-medium">
          <label htmlFor="trip-budget" className="text-gray-500 text-[20px]">
            Trip Budget
          </label>
          <div className="grid grid-cols-3 gap-5 mt-5">
            {TripBudgetList.map((item, index) => (
              <div
                key={index}
                onClick={() => {
                  handleInputChange("budget", item.name);
                  console.log("budget", item.name);
                }}
                className={`p-4 border rounded-xl  hover:shadow-lg cursor-pointer ${
                  formdata?.budget === item.name&&
                    "border-[#f56551] bg-[#f56551]/10"
                }`}
              >
          ...
          <div className="grid grid-cols-3 gap-5 mt-5">
            {SelectTravelesList.map((item, index) => (
              <div
                key={index}
                onClick={() => {
                  handleInputChange("headcount", item.people);
                  console.log("headcount", item.people);
                }}
                className={`p-4 border rounded-xl  hover:shadow-lg cursor-pointer ${
                  formdata?.headcount === item.people &&
                  "border-[#f56551] bg-[#f56551]/10"
                }`}
              >
